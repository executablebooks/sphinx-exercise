...
def test_gated_solution_errors_1(app, warning):
    app.config.exclude_patterns = ["build", "_build", "errors_[2,3]*"]
    try:
        app.build()
...
def test_gated_solution_errors_2(app, warning):
    app.config.exclude_patterns = ["build", "_build", "errors_[1,3]*"]
    try:
        app.build()
    except ExtensionError:
        warnings = getwarning(warning)
...
def test_gated_solution_errors_3(app, warning):
    app.config.exclude_patterns = ["build", "_build", "errors_[1,2]*"]
    try:
        app.build()
    except ExtensionError:
        warnings = getwarning(warning)
